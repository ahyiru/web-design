...
const SearchForm=props=>{
  ...
  return <Form layout="inline" form={form} initialValues={{}} onFinish={submit}>
    {formItems}
    <Form.Item>
...
const Index=props=>{
  ...
  return <div className="list-area">
    {
      (handleBatch?.length||searchList)&&<div className="list-topbar">
        {
          handleBatch?.length&&<div className="list-handle">
            {
              handleBatch.map(v=><Button type="primary" onClick={v.fn} disabled={!selectedRows?.length}>{v.text}</Button>)
            }
          </div>
        }
        {
          searchList&&<div className="list-search">
