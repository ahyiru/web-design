...
const ToolsBar=({actions,defCfg,beforeUpload,imgUrl,rmImg,i18nCfg})=>{
  ...
  const colorMenu=<Menu onClick={handleColorMenuClick}>
    {
      colorCfg.map(item=><Menu.Item key={item.key} icon={<BgColorsOutlined style={{color:type==='draw'?item.key:''}} />}><Tag color={item.key}>{i18nCfg[item.name]}</Tag></Menu.Item>)
    }
  </Menu>;
  const sizeMenu=<Menu onClick={handleSizeMenuClick}>
    {
      sizeCfg.map(item=><Menu.Item key={item.key} icon={<FontColorsOutlined />}>{i18nCfg[item.name]}</Menu.Item>)
    }
  </Menu>;
  return <div className="tools-area">
    <Dropdown.Button trigger={['click']} overlay={colorMenu} icon={<BgColorsOutlined style={{color:type==='draw'?color.key:''}} />}><span style={{color:type==='draw'?color.key:''}}>{i18nCfg[color.name]}</span></Dropdown.Button>
    <Dropdown.Button trigger={['click']} overlay={sizeMenu} icon={<FontColorsOutlined />}><span style={{color:type==='draw'?'var(--red2)':''}}>{i18nCfg[size.name]}</span></Dropdown.Button>
    <Button onClick={()=>{actions.text();setType('text');}}><span style={{color:type==='text'?'var(--red2)':''}}>{i18nCfg.add_text}</span></Button>
    <Button disabled={imgUrl} onClick={()=>{actions.eraser();setType('eraser');}}><span style={{color:type==='eraser'?'var(--red2)':''}}>{i18nCfg.eraser}</span></Button>
    <Button onClick={()=>actions.undo()}>{i18nCfg.undo}</Button>
    <Button onClick={()=>actions.redo()}>{i18nCfg.redo}</Button>
    <Button disabled={imgUrl} onClick={()=>{actions.clean();setType('draw');}}>{i18nCfg.clean_canvas}</Button>
    <Button onClick={()=>{actions.save();/* setType('draw'); */}}>{i18nCfg.save_canvas}</Button>
    <Upload maxCount={1} beforeUpload={file=>beforeUpload(file,()=>setType('draw'))} showUploadList={false}>
      <Button>{i18nCfg.replace_pic}</Button>
    </Upload>
    <Button onClick={()=>{rmImg();setType('draw');}}>{i18nCfg.clean_pic}</Button>
  </div>;
};
...
const Index=props=>{
  ...
  return <div className="page">
    <div className="tools-bar">
      {actions&&<ToolsBar actions={actions} defCfg={defCfg} beforeUpload={beforeUpload} imgUrl={imgUrl} rmImg={()=>setImgUrl(null)} i18nCfg={i18nCfg} />}
    </div>
