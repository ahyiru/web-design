...
const Index=props=>{
  ...
  return <>
    <Form name="login" initialValues={{}} onFinish={onFinish} autoComplete="off">
      ...
      <Form.Item>
        <Button block type="primary" htmlType="submit">{i18nCfg.login}</Button>
      </Form.Item>
      <Form.Item>
        <Button block onClick={()=>onFinish({name:'test1',password:'test1234'})}>{i18nCfg.visitor}</Button>
      </Form.Item>
    </Form>
    <div>
      <div style={{overflow:'hidden'}}>
        <a style={{float:'right'}} onClick={e=>props.router.push('/user/signup')}>{i18nCfg.signup}</a>
        <a style={{float:'left'}} onClick={e=>props.router.push('/user/verifyEmail')}>{i18nCfg.forgetPwd}</a>
      </div>
      <div className="split-line" style={{padding:'15px 0'}}>{i18nCfg.thirdParty}</div>
