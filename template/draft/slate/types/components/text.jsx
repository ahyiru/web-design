...
export const Link=({attributes,children,element})=><a {...attributes} href={element.url}>{children}</a>;
export const Image=({attributes,children,element})=><div {...attributes}><div contentEditable={false}><img src={element.url} alt={element.alt} /></div>{children}</div>;

export const Table=({attributes,children})=><table><tbody {...attributes}>{children}</tbody></table>;
