...
const renderSearchInfo = params => {
  ...
  return (
    <>
      ...
      {description && (
        <Popover title={name} placement="right" content={<Markdown2html description={description} tag={tag} />} key="description">
          <div className={`${styles.description} gray text-overflow`}>{description}</div>
        </Popover>
      )}
    </>
  );
...
const renderStatus = params => {
  ...
  if (changeType) {
    renderItems.push(
      <Popover content={<p className={styles.statusTips}>{changeMessage}</p>} key="changeType">
        <span className={`${styles.itemLable} ${styles.gre}`}>{changeType.toLowerCase()}</span>
      </Popover>,
    );
  }
  if (isDeprecated) {
    renderItems.push(
      <span className={`${styles.itemLable} gray`} key="status">
        已废弃
      </span>,
    );
  }
