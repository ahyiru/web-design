...
export function formatGroupInfoToNodeMeta(groupData, nodeDatas, edges) {
  ...
  if (isCollapsed && edges) {
    ...
    const edgesFromOutside = edges.filter((edge) => {
      ...
      return (includedNodeIds.includes(target.cell.toString()) &&
                !includedNodeIds.includes(source.cell.toString()));
    });
    const edgesToOutside = edges.filter((edge) => {
      const { source, target } = edge;
      return (includedNodeIds.includes(source.cell.toString()) &&
...
export function formatGraphData(graphData = {}) {
  ...
  return {
    nodes: [
      ...formattedNodes,
      ...formattedGroups.filter((group) => !!group?.data?.includedNodes?.length),
    ],
